...
func Ping(dstIP net.IP) (net.HardwareAddr, time.Duration, error) {
	...
	netlink.SetSocketTimeout(1 * time.Second)
	srcIP, iface, err := findUsableInterfaceForNetwork(dstIP)
	if err != nil {
		return nil, 0, err
	}
	return PingOverIface(srcIP, dstIP, *iface)
}
...
func PingOverIface(srcIP, dstIP net.IP, iface netlink.Link) (net.HardwareAddr, time.Duration, error) {
	if err := validateIP(dstIP); err != nil {
		return nil, 0, err
	}
	...
	sock, err := initialize(net.Interface{
		Index:        iface.Attrs().Index,
		MTU:          iface.Attrs().MTU,
		Name:         iface.Attrs().Name,
		HardwareAddr: iface.Attrs().HardwareAddr,
		Flags:        iface.Attrs().Flags,
	})
